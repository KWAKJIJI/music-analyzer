...
두 곡을 업로드해 멜로디, 화성, 리듬 기반 유사도도 계산할 수 있습니다.
...
`multipart/form-data`로 `file_a`, `file_b` 필드에 비교할 음악 파일 두 개를 업로드합니다.
...
```json
{
  ...
  "melody_similarity": 81.2,
  "harmony_similarity": 69.5,
  "rhythm_similarity": 58.8,
  "weights": {
    "melody": 0.5,
    "harmony": 0.3,
    "rhythm": 0.2
  ...
  "summary": {
    "overall": "두 곡의 전체 음악적 특징이 꽤 유사합니다.",
...
- 멜로디 유사도: pitch를 MIDI note로 변환하고 중앙값을 제거한 뒤 DTW 거리로 비교합니다.
- 화성 유사도: chroma 평균 벡터를 12개 키로 회전하며 가장 높은 cosine similarity를 사용합니다.
- 리듬 유사도: BPM 유사도와 onset strength 패턴 유사도를 결합합니다.
- 최종 유사도: 멜로디 50%, 화성 30%, 리듬 20% 가중 평균입니다.

이 점수는 음악적 특징 기반 참고 지표입니다. 법적 표절 여부를 판정하지 않습니다.

## 사용 흐름

1. 단일 곡 분석 탭에서 MP3 또는 WAV 파일을 업로드합니다.
2. 분석 시작 버튼을 누릅니다.
3. 원본 파형, 멜로디 그래프, 화성 그래프, 리듬 그래프를 확인합니다.
4. 두 곡 비교 탭에서 음악 파일 두 개를 업로드합니다.
5. 비교 분석 시작 버튼을 누릅니다.
6. 최종 유사도, 멜로디 유사도, 화성 유사도, 리듬 유사도와 비교 그래프를 확인합니다.
